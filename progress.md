...
> Tally at a glance: **Iterations completed: 18** | **🎯 criteria checked: 2/24** | **Blocked: 0** | **Security fixes applied in iter-09: 15 findings hardened**
...
| 2026-08-03 | Vendor rename + duplicate check-in + carrying checkbox + dashboard drill-down + admin fix | — | Migration 059 (applied live): `visitors.company`→`vendor_name`, `recurring_visits.visitor_company`→`visitor_vendor_name`, `pre_approve_visitor`/`_v2` param renamed to `p_vendor_name` (drop+recreate, ACLs re-granted); all UI/CSV labels now "Vendor". Migration 060 (applied live): partial unique index `visits(visitor_id) where status='checked_in'` blocks duplicate check-in at the DB; `src/lib/activeVisit.ts` added (phone pre-check, ID-proof warning, 23505 handler) wired into guard console + `VisitorForm`/`Kiosk` `checkInPreApproved`. Guard `/visitors`: "Expected" tab removed — `CheckInPanel` now renders unconditionally above tabs, which are just Walk-ins/Inside (default `inside`); legacy `?tab=` values degrade gracefully. Guard dashboard: all 5 KPI tiles now drill down in place (`lib/dashboardDrill.ts`, `lib/useTodayVisits.ts`, `pages/Guard/DashboardDrilldown.tsx`); deleted `lib/useInsideNow.ts` and `pages/Guard/GuardInsideNow.tsx`. `carrying_material` is now an explicit checkbox in `CheckInPhotoStep` gating a discardable remarks field; Reports gained `Carrying`/`Carrying Remarks` columns (screen + CSV), "Not recorded" shown for historical `true` rows with no remarks. Admin "Awaiting an HOD": `HodForm` now opens inline on the card instead of switching to the Departments view. Design tokens: `tokens.css` gained radius scale, motion tokens, hairline border opacity, 3-tier shadow system (light+dark) — no class renamed. 300-line-rule refactors: `pages/Kiosk/useKioskAutoReset.ts` extracted from `Kiosk.tsx`; `tests/unit/pages/Reports.test.tsx` split into `Reports.test.tsx` + `ReportsCarrying.test.tsx`. `supabase/functions/notify-host` updated for the vendor rename. |
| 2026-08-14 | Guard tab rename + KPI tile/list unification + dead code deletion + UI honesty fixes | 1834/1835 (1 pre-existing unrelated `tests/security/realtime.test.ts` live-network timeout) | No new 🎯 criteria — bug fixes and refactors on existing guard surfaces. Guard's second tab renamed "Live Queue"→"Inside Now" (`/guard/inside-now`; `/guard/live-queue` still routable for bookmarks). New `src/lib/guardTiles.ts` gives the dashboard's 4 KPI tiles (Expected Today · Checked In · In Premises · Overstaying) one predicate per tile so the count and its drill-down list can never disagree; "Expected Today" redefined to "approved or walkin_approved, not yet checked in" (was excluding the ordinary booked-ahead case); `useTodayVisits.ts` now ORs in open statuses unbounded, matching `useGateStats`, so a visitor still inside from the day before stays on the list. Dead code deleted: `DashboardSummary.tsx`, `DashboardActivity.tsx`, `DashboardQuickActions.tsx`, `DashboardDrilldown.tsx`, `DashboardTile.tsx`, `lib/recentActivity.ts` + tests — none were imported; dashboard is now `Dashboard.tsx`→`GuardDashboardMain.tsx` with `KpiDrilldownSheet.tsx`. UI honesty fixes: CheckInFrame's "Identity verified" no longer hardcoded green (now requires a captured photo AND a recorded ID type); fabricated `"(parking slot B-12)"` removed from the Vehicle row; visitor pass expiry now resolves `qr_expires_at` → `expected_departure` → `istDayEnd(visitMoment(v))` instead of a hardcoded wrong "06:00 PM"; "Notify Host" no longer appends a magic string to `visits.remarks` (which an HOD reads and Reports prints) — it now inserts a real `visitor_checked_in` notification via new `src/lib/notifyHostCheckIn.ts`. Inside Now check-in details switched from `readOnly <input>` (silently clipped long values) to wrapping `break-words` text; queue table name column lost its `truncate`. New component `src/pages/Guard/CheckInBadgeRail.tsx` extracted from `CheckInFrame.tsx` for the 300-line limit. Visual design frozen per client instruction — logic/naming/correctness only, no restyling (dashboard KPI tiles remain bespoke markup rather than `components/KpiTile.tsx`, a documented divergence). New tests: `tests/unit/lib/guardTiles.test.ts`, `tests/unit/pages/CheckInFrameHonesty.test.tsx`, `tests/unit/pages/CheckInFrameLegibility.test.tsx`, `tests/unit/pages/CheckInBadgeRail.test.tsx`. |
