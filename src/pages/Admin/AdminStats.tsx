// At-a-glance strip above the department list. Read-only summary of the two
// things this page manages, plus the gap worth acting on (departments with no HOD).
...
type Tile = { label: string; value: number; accent: string; icon: React.ReactNode };

export default function AdminStats({ departmentCount, hodCount, unassignedCount }: Props): React.ReactElement {
  const tiles: Tile[] = [
    {
      label: 'Departments',
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      {tiles.map((t, i) => (
        <div key={t.label} className={`stat-card animate-slide-up stagger-${i + 1}`}>
