...
export default function OverviewNotifications({ loading, notifs, onMarkRead, onDismiss }: Props): React.ReactElement {
  return (
          ...
          {notifs.map((n) => {
            ...
            return (
              <div key={n.id} className={`px-5 py-4 transition-colors ${isUnread ? 'bg-brand-50/40 dark:bg-brand-500/[0.04]' : ''}`}>
                ...
                <Link to="/approvals" className="text-xs font-semibold text-brand-600 dark:text-brand-400 hover:underline mt-1.5 ml-4 block">
                  More information →
                </Link>
              </div>
            );
