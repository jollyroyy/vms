...
export default function WhosInside(): React.ReactElement {
  ...
  const load = useCallback(async () => {
    ...
    // show when each visitor was approved, so they have to be attached here.
    const withActors = await attachVisitActors(raw);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="page-subtitle flex items-center gap-2">
              ...
              {visits.length} visitor{visits.length !== 1 ? 's' : ''} in or due today
            </p>
          </div>
        </div>
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={() => exportToCsv(toReportRows(displayed), `whosinside-${today}.csv`)} className="no-print btn-secondary text-sm flex items-center gap-2" title="Export CSV">
