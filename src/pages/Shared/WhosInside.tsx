...
type ActiveTab = 'checked_in' | 'pre_approved';
...
export default function WhosInside(): React.ReactElement {
  ...
  const displayed = tab === 'checked_in' ? checkedIn : preApproved;
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="page-subtitle flex items-center gap-2">
            ...
            {checkedIn.length} inside · {preApproved.length} pre-approved{pending.length > 0 ? ` · ${pending.length} pending` : ''}
          </p>
        </div>
      ...
      {!loading && displayed.length === 0 && !error && (
        <div className="empty-state py-20">
          ...
          <p className="text-lg font-medium text-navy-500">
            {tab === 'checked_in' ? 'No visitors inside' : 'No pre-approved visitors'}
          </p>
          <p className="text-sm text-navy-300 mt-1">
            {tab === 'checked_in' ? 'Checked-in visitors will appear here' : 'Pre-approved visitors waiting to check in will appear here'}
          </p>
        </div>
      )}
