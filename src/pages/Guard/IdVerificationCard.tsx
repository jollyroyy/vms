...
export default function IdVerificationCard({ idTarget, initialsOf, onDeny }: IdVerificationCardProps): React.ReactElement {
  return (
        ...
        <div className="flex gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <Link
                // "/guard/preregistered?checkin=<id>", NOT the Entry & Exit
                // tab. ID scanning exists ONLY inside a check-in flow
                // (CheckInPhotoStep -> IdScanOverlay), so this button has to
                // arrive somewhere that can actually perform a scan. It used
                // to point at /guard/inside-now?verify=, which renders a
                // read-only CheckInFrame with no scan control at all — and
                // once that tab narrowed to visitors who have already been
                // through the gate, it could not even FIND this visitor
                // (they have no checked_in_at yet), so the guard landed on a
                // list with nothing selected. A button labelled "Verify ID"
                // must open the thing that verifies an ID.
                to={`/guard/preregistered?checkin=${idTarget.id}`}
