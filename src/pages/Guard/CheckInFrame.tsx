  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
      ...
      <div className="xl:col-span-4 rounded-2xl bg-surface-100/60 dark:bg-white/[0.03] border border-surface-200/60 dark:border-white/[0.07] p-5 shadow-glow-sm">
        ...
        {/* Printable pass, mirrored from the reference screen: white rounded
            card · dark top notch · issuing-company logo block · bold blue
            "VISITOR PASS" band · circular photo with blue ring · dark name ·
            blue "Day Pass #…" · gray validity note · QR code. */}
        <div
          id="vms-print-badge"
          className="rounded-2xl bg-white p-5 shadow-lg border border-surface-200 dark:border-white/20 flex flex-col items-center">
          {/* Dark lanyard notch */}
          ...
          <div className="flex flex-col items-center">
            ...
            <p className="text-[#111827] font-display font-extrabold text-base tracking-wide">Quest Mall</p>
            <p className="text-[9px] font-bold uppercase tracking-[0.22em] text-[#5a6070]">Corporate Park</p>
          </div>

          {/* Bold blue band, exactly as the reference */}
          <div className="mt-3 w-full rounded-lg bg-[#1d4ed8] px-4 py-2.5 text-center">
            <p className="text-white font-display font-bold tracking-[0.14em] text-base uppercase">Visitor Pass</p>
          </div>

          <div className="flex flex-col items-center gap-2 pt-4">
            <div className="w-20 h-20 rounded-full overflow-hidden border-[3px] border-brand-500 p-0.5 bg-white">
              <div className="w-full h-full rounded-full overflow-hidden">
                {activeVisit.photo_data ? (
                  <img src={activeVisit.photo_data} alt={activeVisit.visitor?.full_name ?? 'Visitor'} className="w-full h-full object-cover" />
                ) : (
                  <span className="w-full h-full flex items-center justify-center text-sm font-bold text-[#8b93a5]">
                    {initialsOf(activeVisit.visitor?.full_name)}
                  </span>
                )}
              </div>
            </div>
            <p className="font-display font-bold text-[#111827] text-lg leading-tight text-center">
              {activeVisit.visitor?.full_name ?? 'Visitor'}
            </p>
            <p className="text-[13px] font-bold text-brand-600">Day Pass #{activeVisit.ref_number.slice(-4)}</p>
            <p className="text-[11px] font-medium text-[#5a6070]">Valid until 06:00 PM</p>
            {qrDataUrl ? (
              <img src={qrDataUrl} alt="QR code" className="mt-1 w-24 h-24" />
            ) : (
              <div className="mt-1 w-24 h-24 border-2 border-[#d9dde5] rounded-lg flex items-center justify-center text-xs font-bold text-[#8b93a5]">QR</div>
            )}
          </div>
        </div>
