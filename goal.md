...
- [ ] 🎯 **M12-GUARD**: Guard console shows active visits, register form, exit log; handles check-in/check-out state. VisitorForm loads department contacts (profiles with matching department_id) when department is selected; shows full_name in the "Person to Meet" dropdown. `/visitors` is now CheckInPanel (unconditional) + Walk-ins/Inside tabs only (2026-08-03 — the old "Expected" tab was CheckInPanel with extra steps); Dashboard KPI tiles drill down in place via `lib/dashboardDrill.ts` instead of navigating away.
...
**2026-08-03 change set — new edge cases (duplicate check-in, carrying checkbox, guard/dashboard restructure, vendor rename):**
- RED EC-44: Visitor already inside tries to check in again — via the guard console, via the kiosk, or via a second guard device racing the first. `lib/activeVisit.ts` pre-checks by phone and warns by ID-proof, but the real backstop is migration 060's partial unique index on `visits(visitor_id) where status='checked_in'`; a raced pre-check surfaces as Postgres `23505` on `visits_one_open_per_visitor`, caught by `isAlreadyInsideError()` and reworded, never a raw DB error.
- RED EC-45: Two different people whose ID cards share the same `id_type` + `id_last4` — only 4 digits are stored, so `findActiveVisitByIdProof` is a *warning* surfaced to the guard, not a block; there is deliberately no unique index behind it (would generate false collisions).
- YELLOW EC-46: Visitor with no phone on file, or a phone `normalizePhone()` can't parse — `findActiveVisitByPhone` catches the throw and returns `null`; a guard must never see "already inside" for a visitor the system couldn't identify.
- RED EC-47: `carrying_material = true` on rows written before `carrying_remarks` existed (migration 058) — Reports shows "Not recorded", never a blank cell, so it reads as "we know they were carrying something, just not what."
- YELLOW EC-48: Guard ticks the carrying checkbox, types a list, then unticks before confirming — `CheckInPhotoStep` discards the typed remarks; the box, not the text, is the record.
- YELLOW EC-49: Legacy `?tab=` deep link or bookmark to a guard `/visitors` tab that no longer exists (`checkin`, `exit`, `no-show`, `checked-out`, `rejected`, `all`) — must degrade to the nearest live tab (Walk-ins/Inside), never 404 into a blank one.
- YELLOW EC-50: A department with zero HODs vs. every department covered — Admin's "Awaiting an HOD" drill-down must show both states without the "Assign HOD" action swapping the filtered card list out for the unfiltered Departments view (the bug `HodForm` inline-editing fixed).
- RED EC-51: `entered` and `inside` collapsing into the same number — `entered` is `checked_in_at IS NOT NULL` (cumulative), `inside` is `status === 'checked_in'` (live); the invariant `entered === inside + checkedOut` must hold across every dashboard drill-down, not just the KPI tiles (see CLAUDE.md guard console notes).
- YELLOW EC-52: A realtime refresh arrives while a KPI tile is expanded on the dashboard — the drill-down panel must not flash or collapse/reset; it re-filters the same open `DrillKey` against the refreshed visit list.
- YELLOW EC-53: Migration 059's rename forces a drop-and-recreate of `pre_approve_visitor`/`pre_approve_visitor_v2` (Postgres won't rename a function parameter via `CREATE OR REPLACE`) — the migration must re-grant the exact prior ACLs (`anon`+`authenticated`+`service_role` on v1, `authenticated`+`service_role` on v2) or the drop silently revokes access.
...
| 2026-08-03 | — | v2.3: Vendor rename (migration 059, `company`→`vendor_name`), guard `/visitors` restructured to Walk-ins/Inside only (CheckInPanel now unconditional above the tabs), guard dashboard KPI tiles gained in-place drill-down (`lib/dashboardDrill.ts`), carrying material became an explicit checkbox with a discardable remarks field, duplicate check-in blocked at the DB (migration 060) with a guard-readable pre-check (`lib/activeVisit.ts`), Admin "Awaiting an HOD" no longer loses its filtered view when assigning; §8.4 gained EC-44..EC-53 | Reflects the 2026-08-03 change set landed in CLAUDE.md; see PRD.md FR-VIS-10 and CLAUDE.md "Guard console" / "Admin scope" sections for full detail |
